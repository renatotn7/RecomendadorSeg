...
import com.bradseg.seguros.model.ArffAttribute;
import com.bradseg.seguros.model.ArffInstance;
...
public class ArffDataConvertor extends AbstractDataConvertor {
    ...
    public ArffDataConvertor(String path) {
        dataPath = path;
        instances = new ArrayList<ArffInstance>();
        attributes = new ArrayList<ArffAttribute>();
        columnIds = new ArrayList<BiMap<String, Integer>>();
     ...
     * @throws IOException
     */
    private void dataReader(BufferedReader rd) throws IOException {
        ...
        for (int row = 0; row < numRows; row++) {
            ...
            for (int i = 0; i < numAttrs; i++) {
                if (i == userCol) {
                    ...
                    double userId =  Double.parseDouble((String)instance.getValueByIndex(userCol));
                    ...
                    int j = i > ratingCol? i - 1: i;
                    double itemId = Double.parseDouble((String)instance.getValueByIndex(itemCol));
                    ...
                    if (attrType.equals("STRING")) {
                    	...
                    	String strFeatureId="";
                    	if(instance.getValueByIndex(i) instanceof Integer) {
                    		 int val = (int) instance.getValueByIndex(i);
                    		 strFeatureId = String.valueOf((int) val);
                    	}else 	if(instance.getValueByIndex(i) instanceof Double) {
		                   		 double val = (double) instance.getValueByIndex(i);
		                   		 strFeatureId = String.valueOf((int) val);
		                    		}
                    	else 	if(instance.getValueByIndex(i) instanceof String) {
	                   		 String val = (String) instance.getValueByIndex(i);
	                   		 strFeatureId = String.valueOf(Integer.parseInt(val));
	                    		}
